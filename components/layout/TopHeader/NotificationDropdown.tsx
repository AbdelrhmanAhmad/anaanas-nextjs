          ...
          <CardBody className="p-0">
            <ul className="list-group list-group-flush list-unstyled p-2">
              {allNotifications.slice(0, 4).map((notification, idx) => (
                <li key={idx}>
                  <div className={clsx('rounded d-sm-flex border-0 mb-1 px-3 py-md-3 py-2 position-relative', { 'badge-unread': !notification.isRead })}>
                    ...
                    <p className="small text-nowrap d-md-block d-none">{timeSince(notification.time).slice(0, 5)}</p>
                  </div>
                </li>
              ))}
            </ul>
